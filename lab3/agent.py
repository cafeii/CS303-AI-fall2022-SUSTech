...
class ProblemSolvingAgent:
    ...
    def Astar(self, obstacles, start_pos, goal_pos):
        ...
        forniter = []
        heapq.heappush(forniter, (self.euclidean_distance(start_pos, goal_pos) + cost, start_pos))
        parent = {goal_pos: None}

        while len(forniter) != 0:
            point = heapq.heappop(forniter)
            state = point[1]
            ...
            if state not in visited:
                ...
                nei = [x for x, _ in list(self.neighbours_of(obstacles, state))]
                for pos in nei:
                    f = [x[1] for x in forniter]
                    n = cost - self.euclidean_distance(state, goal_pos) + self.euclidean_distance(pos, state) + self.euclidean_distance(pos, goal_pos)
                    if pos not in f and pos not in visited:
                        heapq.heappush(forniter, (n, pos))
                        parent.update({pos: state})
                    elif pos in f:
